import upload_svg from './svg/upload.svg'
import newsfeed from './svg/newsfeed.svg'

import make_vote from './images/make_vote.png'
import create_vote from './images/create_vote.jpg'
import newsfeed_vote from './images/vote-page.jpg'
import app_logo from './images/app_logo.png'
import createVote from './images/createVote.png'
import homeIcon from './images/homeIcon.png'
import search from './images/search.png'
import add_btn from './images/add_btn.png'
import vote_icon from './images/vote.png'
import vote_placeholder from './images/votePlaceHolder.png'
import vote_card from './images/voteCard.png'
import vote_candidate from './images/voteCandidate.png'
import back from './images/back.png'
import history from './images/history.png'
export {
    search, upload_svg, newsfeed,
    make_vote, vote_icon, create_vote,
    newsfeed_vote, app_logo, createVote,
    homeIcon, add_btn, vote_placeholder,
    vote_candidate, vote_card, back,
    history
}